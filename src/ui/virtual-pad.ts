 ...
 *
 * angle 90 でノッチが左・270 で右。どちらも実機で確認済み(逆向きにすると
 * スティックとボタンの大小が入れ替わることを実機で確認)。
 *
 * 触らない条件を広く取ってあるので、ノッチの無い端末は素通りする:
 *   - 左右が 0 (ノッチ無し端末・iPad・Chrome等のブラウザ表示) → そのまま
 *   - 左右が同値でない (Android のパンチホール等、値が正確に返る場合) → そのまま
 *   - 縦向き(angle 0/180)・角度が取れない・想定外の角度 → そのまま(安全側)
 * 残る危険は「左右とも本当に塞がっている端末」で左右同値が返る場合だけで、
 * これは手元に実機が無く未検証。
 */
