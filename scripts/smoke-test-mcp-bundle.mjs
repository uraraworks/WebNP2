...
// ブリッジの既定ポート(3098)は開発中の実サーバーと衝突しうるので別ポートを使う。
const BRIDGE_PORT = '13098';
const TIMEOUT_MS = 20000;

const child = spawn(process.execPath, [target], {
  env: { ...process.env, WEBNP2_BRIDGE_PORT: BRIDGE_PORT },
...
send({
  ...
  params: {
    ...
    capabilities: {},
    clientInfo: { name: 'webnp2-smoke-test', version: '1.0.0' },
  },
});
...
const check = () => {
  ...
  for (const name of ['screen_text', 'type_text', 'send_keys', 'screenshot']) {
